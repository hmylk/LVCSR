...
#include "../../../_sharelib/src/decoding/slflatt.h"
...
public:
	...
	Boolean readHTK(File &file, HTKHeader *header = 0, Boolean useNullNodes = false);
	Boolean loadFromHTK(HTKLattice *hLat, HTKHeader2 *hHeader, HTKHeader *header = 0, Boolean useNullNodes = false);
	...
	void setSkipTune(bool bSkipTune){skipTune = bSkipTune;}
...
protected: 
    ...
    Boolean top;                // an indicator for whether two null nodes 
				...
				// to <s> and </s>.
	int splitPhones(const char *phoneString, Array<char *> &monophones, Array<char *> &phones, Array<NBestTimestamp> &phoneDurs);
