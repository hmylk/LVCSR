...
double LOGMath::Add(const double x,const double y) 
{
	double z=x+y;
	return (z<LOGSMALL)?LOGZERO:z;
}

double LOGMath::Sub(const double x,const double y) 
{
	double z=x-y;
	return (z<LOGSMALL)?LOGZERO:z;
}

double LOGMath::Log(const double x) 
{
	ASSERT2(x>=0,"log(%g)",x);
	return (x<=MINLARG)?LOGZERO:log(x);
}

double LOGMath::Exp(const double x) 
{
	return (x<=MINEARG)?0.0:exp(x);
}

double LOGMath::LogAdd(const double x,const double y) 
{
...
double LOGMath::LogSub(const double x,const double y) 
{
...
// 2003.11.25  : add
float LOGMath::Add(const float x,const float y) 
{
	float z=x+y;
	return (float) ((z<LOGSMALL)?LOGZERO:z);
}

float LOGMath::Sub(const float x,const float y) 
{
	double z=x-y;
	return (float)((z<LOGSMALL)?LOGZERO:z);
}

float LOGMath::Log(const float x) 
{
	ASSERT2(x>=0,"log(%g)",x);
	return (float) ((x<=MINLARG)?LOGZERO:log(x));
}

float LOGMath::Exp(const float x) 
{
	return (float) ((x<=MINEARG)?0.0:exp(x));
}

float LOGMath::LogAdd(const float x,const float y) 
{
...
float LOGMath::LogSub(const float x,const float y) 
{
