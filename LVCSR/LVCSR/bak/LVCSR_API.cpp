...
LVCSR_ProcessEventCallback *pEventCallBack = NULL; // 状态事件回调
...
LVCSR_ERROR CallbackSessionResult(int sessionId);
...
LVCSR_API_API int LVCSR_Init(const char* sysdir,const char*  cfgFile,int NumberOfSession)
{
	...
	pEventCallBack = (LVCSR_ProcessEventCallback*)malloc(sizeof(LVCSR_ProcessEventCallback)*sessionNum);
	if(g_IndexWithRec)
	{
...
LVCSR_API_API int LVCSR_Start(int sessionId /*=0*/)
{
	...
	if (sessionId >= 0)
	{
		...
		if (pEventCallBack[sessionId]) (*pEventCallBack[sessionId])(LVCSR_EVENT_START_COMPLETE, sessionId);
		return sessionId;
	}
...
LVCSR_API_API int LVCSR_Stop(int sessionId)
{
	...
	if (pEventCallBack[sessionId]) (*pEventCallBack[sessionId])(LVCSR_EVENT_STOP_COMPLETE, sessionId);
	pResultCallBack[sessionId] = NULL;   // 清除设置的回调函数
	pEventCallBack[sessionId] = NULL;
	if (g_IndexWithRec)
	{
...
LVCSR_API_API int LVCSR_SetEventCallbackFunc(LVCSR_ProcessEventCallback pFunc, int sessionId)
{
	if (!(SessionPool->IsACellWork(sessionId))) return LVCSR_ERROR_COMMON;
	pEventCallBack[sessionId] = pFunc;
	ApiLog->logoutf2("session %d: Set EventCallback Function\n", sessionId);
	if (pEventCallBack[sessionId]) (*pEventCallBack[sessionId])(LVCSR_EVENT_START_COMPLETE, sessionId);   // just to fit tbnrsdidemo
	return LVCSR_SUCCESS;
}
...
int DecodeATask(float *newSet, int frameNum, DWORD SessionId, DWORD SavedFileIdx, DWORD DataTimestamp, DWORD DataTimestampEnd, int decoderId, SOUND_TYPE soundType, int SegIdxInFile)
{
	...
	if (newSet!=NULL && frameNum>0) 
	{
		...
		newSet2 = new float[(frameNum*(ElementsInSrcFeature)+1)*sizeof(float)];
		memset(newSet2,0,(frameNum*(ElementsInSrcFeature)+1)*sizeof(float));
		//newSet2=(float *)Malloc32(frameNum*(ElementsInSrcFeature)+1, int(sizeof(float) ), true);
		if (NULL == newSet2)
		{
		...
		{
			 ApiLog->logoutf("no enter to  GetCN  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");   
			 pResultArray = NULL;
			if (g_IndexWithRec)
			   pWordResultArray = NULL;
		}
#endif
	}
	...
	WordResult* pResultSrc =NULL;
	if (g_IndexWithRec&&!strcmp(CNFormat,"KWS")) //关键词+识别
	{
		pResultSrc=(WordResult*)pWordResultArray;
	}
	else if (!g_IndexWithRec&&!strcmp(CNFormat,"DaLian"))//纯识别模式
	{
		pResultSrc=(WordResult*)pResultArray;
	...
	else if (!g_IndexWithRec&&!strcmp(CNFormat,"KWS"))//纯关键词模式
	{
 	...
 	{
 		ApiLog->logoutf("session %d: %s\n",SessionId,pResultSrc->cands->text);
 		pResultCopy= (WordResult*)malloc(sizeof(WordResult)*1);
 		pResultCopy->cands = NULL;
 		...
 		if (lenText == 0)
 		{
			pResultCopy->cands= (ACand *)malloc(sizeof(ACand)*1);
			pResultCopy->cands->text = new char[1];
			pResultCopy->cands->phone = new char[1];
			pResultCopy->cands->segTime=new char[1];
			*(pResultCopy->cands->text)='\0';
			*(pResultCopy->cands->phone)='\0';
			*(pResultCopy->cands->segTime)='\0';
 			pResultCopy->next = NULL;
 			pResultCopy->candNum = 1;
 			pResultCopy->cands->next= NULL;
 		}
 		else
 		{
 			pResultCopy->candNum = pResultSrc->candNum;
 			int candsNum = pResultSrc->candNum;
 			int cn=0;
 			ACand* curCpyCand = pResultCopy->cands;
 			...
 			while(curSrcCand!=NULL)//构建多候选链表
 			{
                ...
                //printf("pResultCopy->cands->phone %s\n",newACand->phone);

 				newACand->segTime=new char[lenSegtime+1];
 				//ApiLog->logoutf("pResultCopy->cands->segTime [%0x]\n",newACand->segTime);
 				...
 				if(pResultCopy->cands==NULL)
 				{
 					pResultCopy->cands = newACand;
 					curCpyCand=newACand;
 				}
 				else
 				{
	...
	if (!stricmp(CNFormat, "DaLian"))
	{
		...
		int n=0;
		WordResult *tempw = (WordResult *)pResultCopy;
		if(pResultCopy != NULL&&tempw->cands!=NULL&&tempw->cands->text!=NULL)
		{
	...
	if (retn > 0)
	{
		...
		int temp = aResult->saveFileIdx + 1;
...
int CallBackResult(void* lpParam)
{
	...
	while(numberOfSentences > 0) 
	{
		...
		{	
			...
			if (bOnlineMode)//在线情况下，分句返回结果
			{
				ret = CallbackSessionResult(SessionId);
		...
		if (pEventCallBack[SessionId] && Buffer_[SessionId]->bEndRecord && (Buffer_[SessionId]->SavedFileIdx == SavedFileIdx + numberOfSentences))
		{
			if (!bOnlineMode) //离线情况下，整个wav一次返回
			{
				ret = CallbackSessionResult(SessionId);
				free(parameter),parameter=NULL;
			}
			
			(*pEventCallBack[SessionId])(LVCSR_EVENT_RECOGNITION_COMPLETE, SessionId);
...
LVCSR_ERROR CallbackSessionResult(int sessionId)
{
	...
	{
		(*pResultCallBack[sessionId])(Buffer_[sessionId]->pResultLink, sessionId);
	}

	/*释放开辟的识别结果内存*/
	temp = Buffer_[sessionId]->pResultLink;
	while(temp!=NULL)
	{
		if (temp->speech_type==RT_VOICE)
		{
	...
	/*释放引擎本身task内存*/
	iterSent = Buffer_[sessionId]->vecSortedResult.begin();
	while(iterSent!=Buffer_[sessionId]->vecSortedResult.end())
	{	
		if ((*iterSent)->newSet)
		{
