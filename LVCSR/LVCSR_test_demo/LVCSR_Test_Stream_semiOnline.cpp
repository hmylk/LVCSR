...
#endif
void myTBNR_ResultCallback( FeSegInfo *pResultArray,bool bOver, int sessionId);    //recognize result callback function
...
#endif
{
	...
	while (1)
	{
		...
		short *WavAllData = new short[FileSize[SessNum]/2];
		memset(WavAllData,0,FileSize[SessNum]);
		int tempRead = fread(WavAllData, 2, FileSize[SessNum]/2, TestFile[SessNum]);
		...
		while (1)
		{
			...
			int segLen = segInfo->end_time-segInfo->start_time;
			short *tmpData = new short[segLen];
			memset(tmpData,0,segLen);
			memcpy(tmpData,WavAllData+segInfo->start_time,segLen*sizeof(short));
			FeSpeechAttri  dataAttr;
			dataAttr.data_buf = tmpData;
			dataAttr.dataLen = segLen;
			dataAttr.send_status = FE_SEND_ALL;
			if (segInfo->detect_status==RT_VOICE)
			{
		...
		delete[] WavAllData,WavAllData=NULL;
...
void myTBNR_ResultCallback(FeSegInfo *pResultArray,bool bOver, int sessionId)
{
